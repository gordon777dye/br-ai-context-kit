...
1. Read app/BR_launch.md to extract the environment variable definitions
...
- **Always-load** when reading or writing this app's code:
  ...
  - [`../app/BR_launch.md`](../app/BR_launch.md) — BR launch environment + build / run / test / deploy commands
...
| **This app's** build / run / test commands + BR launch env | [`../app/BR_launch.md`](../app/BR_launch.md) — *always-load* |
...
Before running BR check the BR configuration file (specified in BR_launch.md) to see where the `LOGGING` 
...
described in [`../app/BR_launch.md`](../app/BR_launch.md).

Use a BR config with **no auto-launch of the app menu** (`brconfig.ai_util`), so BR runs your initial
command (e.g. `PROC <procname>`) unattended; `BR_launch.md` covers creating such an "AI access" 
